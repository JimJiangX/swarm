...
func updateConfigAfterUpdateResource(ctx context.Context, svc *Service, units []*unit, memory *int64) error {
	...
	if svc.spec.Image.Name == "redis" || svc.spec.Image.Name == "upredis" {
		...
		kv.fullName = "mysqld::" + kv.key
		kv.value = strconv.Itoa(int(n))
	}

	configs, err := svc.reloadServiceConfig(ctx, "")
	if err != nil {
		return err
	}
	...
	for i := range units {
		for c := range configs {
			if units[i].u.ID != configs[c].ID {
				continue
			}

			if configs[c].Keysets == nil {
				configs[c].Keysets = []structs.Keyset{{
					Key:   kv.fullName,
					Value: kv.value,
				}}
			} else {
				configs[c].Keysets = append(configs[c].Keysets, structs.Keyset{
					Key:   kv.fullName,
					Value: kv.value,
				})
			}
	...
	cms, err := svc.pc.UpdateConfigs(ctx, svc.ID(), configs)
	if err != nil {
		return err
	}
