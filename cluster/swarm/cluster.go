...
func (c *Cluster) createContainer(config *cluster.ContainerConfig, name string, withImageAffinity bool, authConfig *types.AuthConfig) (*cluster.Container, error) {
	...
	if err != nil {
		logrus.WithFields(logrus.Fields{"NodeName": n.Name, "NodeID": n.ID}).WithError(err).Error("Failed to create container")
	} else {
		containerFlag := name
		if containerFlag == "" {
			containerFlag = stringid.TruncateID(container.ID)
		}
		logrus.WithFields(logrus.Fields{"NodeName": n.Name, "NodeID": n.ID}).Debugf("Scheduling container %s to ", containerFlag)
	}
