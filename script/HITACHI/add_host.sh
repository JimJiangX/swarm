...
if [ -z $tempfile ]; then
	...
	do
		...
		if [ "${ret}" == "1" ]; then
			...
			if [ "${ret}" == "0" ]; then
				echo "sudo raidcom add host_grp -I${Instance_ID} -port ${port} -host_grp_name ${hostname}"
				if [ $? -ne 0 ]; then
			...
			echo "sudo raidcom add hba_wwn -I${Instance_ID} -port ${port} ${hostname} -hba_wwn ${wwn}"
			if [ $? -ne 0 ]; then
